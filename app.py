...
def pdf_to_excel_tool():
    ...
    try:
        ...
        # Use pdfplumber to find and extract tables from all pages
        with pdfplumber.open(input_path) as pdf:
            with pd.ExcelWriter(excel_filepath, engine='openpyxl') as writer:
                ...
                if not found_table:
                    # If no tables are found, extract text as a fallback
                    app.logger.warning("No tables found, extracting raw text.")
                    for i, page in enumerate(pdf.pages):
...
def office_to_pdf_tool():
    """A single function to handle Word, Excel, and PowerPoint to PDF."""
    if 'file' not in request.files: return jsonify({"error": "No file part"}), 400
    file = request.files['file']
    if file.filename == '': return jsonify({"error": "No selected file"}), 400
    ...
    try:
        ...
        # LibreOffice command works for .docx, .xlsx, .pptx
...
# Specific routes that call the main office converter
